# Single use case perspective How many child use cases were generated by the use case,
# how many coverage were increased, how many were increased respectively, and how many parent use cases were increased in total
# The parent use case uses various variation methods to improve the effect
import json
...
def printCovByRaise(raise_rate_dic):
    ...
    for file_key in raise_rate_dic:
        # print(file_key)
        if raise_rate_dic[file_key]['functions_percent']['old_cov'] and raise_rate_dic[file_key]['functions_percent'][
            'new_cov']:
            print("-" * 30 + f"{file_key}" + "-" * 30)
            for type_key in raise_rate_dic[file_key]:
                print('%s increased from %.2f to %.2f by %.2f%%' % (
